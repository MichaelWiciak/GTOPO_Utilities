...
int main(int argc, char **argv){
  ...
  int code = checkArgsErrorsForAssembleReduced(argc, argv[0]);
  if (code!=999){
    // If not, terminate with error code
    return code;}
  code = 0;
  // Calculate output Image width and height
  div_t numberOfImages = div((argc - 5),5);
  ...
  for (int imageIndex=0; imageIndex<imageNumber; imageIndex++){
    FILE *inputFile = fopen(argv[5 + 2 +(imageIndex*5)], "r");
    if (inputFile==NULL){
      // If not, terminate with error code
      printErrorStringAndDescription(EXIT_BAD_INPUT_FILE, argv[5 + 2 +(imageIndex*5)]);
  ...
  // Read the image data of each image one by one
  ...
  for (int imageIndex=0; imageIndex<imageNumber; imageIndex++){
    ...
    FILE *inputFile = fopen(argv[5 + 2 +(imageIndex*5)], "r");
    code = readFile(argv[5 + 2 +(imageIndex*5)],fileValuesInput, inputFile);
		fileValuesInput->width = atoi(argv[5 + 3 + (imageIndex*5)]);
		fileValuesInput->height = atoi(argv[5 + 4 + (imageIndex*5)]);
    // Check if that image can be read
    if (code!=0){
      // If not, terminate with error code
      return code;}
    long nImageBytesInput = fileValuesInput->width * fileValuesInput->height * sizeof(short);
		code = readImageDataBinary(fileValuesInput, nImageBytesInput, inputFile, argv[5 + 2 +(imageIndex*5)]);
    // Check if successfully written imageData
    if (code !=0){
      return code;}
    closeTheFile(inputFile);
    // Store the image values in arrays
    rowStart[imageIndex] = atoi(argv[5 + 0 +(imageIndex*5)]);
    rowEnd[imageIndex] = atoi(argv[5 + 0 +(imageIndex*5)])+fileValuesInput->height;
    colStart[imageIndex] = atoi(argv[5 + 1 +(imageIndex*5)]);
    colEnd[imageIndex] = atoi(argv[5 + 1 +(imageIndex*5)]) + fileValuesInput->width;
    ...
    for (short *nextGrayValue = fileValues->imageData; nextGrayValue < fileValues->imageData + nImageBytes; nextGrayValue++){
      ...
      *nextGrayValue = *nextGrayValue;
      ...
      if (column==(fileValues->width-1)){
        ...
        column+=1;}}}
  // Check if the factor supplied by user is a valid integer
  code = checkIfinteger(argv[4]);
  if (code!=0){
    return code;}
  // Converts factor string to integer
  unsigned int factor = atoi(argv[4]);
  // Write the file in a reduced format
  code = writeASCIIDataInBinaryReduced(outputFile, fileValues, argv[1], nImageBytes, factor);
  // Check if written correctly to file
  if (code!=0){
    // If error, return the error code
    return code;}
  // Program successfully performed assemble reduce
